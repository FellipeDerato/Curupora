    ...
    public partial class Inspector
    {
        public class InspectorComponentView : List<Editor>
        {
            ...
            void PageComponentHeader(Editor editor)
            {
                ...
                #region MODIFICATION ICON
                var iconField = editor.target.GetType().GetField("CustomIcon",
                    BindingFlags.Public | BindingFlags.Instance);

                if (iconField != null)
                {
                    var customIcon = iconField.GetValue(editor.target) as Texture2D;
                    if (customIcon != null)
                    {
                ...
                #region MODIFICATION NAME
                var field = editor.target.GetType().GetField("CustomName",
                    BindingFlags.Public | BindingFlags.Instance);

                if (field != null)
                {
                    var value = field.GetValue(editor.target) as string;
                    if (!string.IsNullOrEmpty(value))
                    {
                        name = name.Replace("GameEvent", " ");
