    ...
    public partial class Inspector
    {
        public class InspectorGridView : /*List<Row>*/List<Editor>
        {
            ...
            GUIContent GetContent(Editor editor)
            {
                ...
                // Se tiver um campo CustomIcon, usa ele
                var field = value.GetType().GetField("CustomIcon",
                    BindingFlags.Public | BindingFlags.Instance);

                if (field != null)
                {
                    var customIcon = field.GetValue(value) as Texture2D;
                    if (customIcon != null)
                    {
            ...
            bool Button(Object value, bool selected)
            {
                ...
                // Custom Color
                var field2 = value.GetType().GetField("CustomCollor",
                    BindingFlags.Public | BindingFlags.Instance);

                if (field != null)
                {
                    var customEnum = field2.GetValue(value);
                    if (customEnum != null)
                    {
                        Color cor = ((Colors)customEnum).ToUnityColor();
                        var oldColor = GUI.backgroundColor;
                        GUI.backgroundColor = cor;
                        bool pressed = GL.Button(content, style, slotButtonWidth, slotButtonHeight);
